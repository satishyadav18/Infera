...
const GenerateImages = () => {
  ...
  const imageStyle = [ 'Realistic', 'Ghibli style','Anime style', 'Cartoon style', '3D style', 'Fantasy style', 'Portrait style' ]
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <div className='mt-6 flex items-center gap-2'>
          <label className='relative cursor-pointer'>
            ...
            <div className={`w-9 h-5 rounded-full ${publish ? 'bg-green-500' : 'bg-gray-300'} transition-all duration-200`}></div>
            <span className={`absolute left-1 top-1 w-3 h-3 rounded-full bg-white transition-all duration-200 ${publish ? 'translate-x-4' : ''}`}></span>
          </label>
        ...
        {!content ? (
          <div className='flex-1 flex justify-center items-center'>
            <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
              ...
              <p className='text-gray-500'>Describe your image and click "Generate Image" to get started</p>
            </div>
          </div> 
        ) : (
